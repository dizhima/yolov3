...
def create_modules(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            yolo_layer = YOLOLayer(anchors, img_size)
...
class RelationNetwork(nn.Module):
    def __init__(self, input_size=2048, hidden_size=1024):
        ...
        self.rn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(hidden_size, 1, 1, 1, bias=False)
        )
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, emb_ft, x, targets=None, img_dim=None):
        ...
        # prediction size 
        # x.size() = (b_size, (num_ft+5)*num_anchors, grid_size, grid_size)
        # >>> (b_size, num_anchors, grid_size, grid_size, numcls+5)
        # eg. torch.Size([16, 3, 60, 60, 1029])
        prediction = (
            x.view(num_samples, self.num_anchors, self.num_ft+5, grid_size, grid_size)
            .permute(0, 1, 3, 4, 2)
            .contiguous()
        )
        ...
        s_emb = s_emb.repeat(num_samples,self.num_anchors,1,grid_size,grid_size)
        v_emb = prediction[...,5:].permute(0,1,4,2,3).contiguous().repeat(1,1,self.num_anchors,1,1)
        print(v_emb.size(), s_emb.size())
        ...
        pred_cls = torch.sigmoid(relation)  # Cls pred.
...
class Darknet(nn.Module):
    ...
    def forward(self, emb, x, targets=None):
        img_dim = x.shape[2]
        loss = 0
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                x, layer_loss = module[0](emb, x, targets, img_dim)
                loss += layer_loss
